...
current_year = datetime.now().year
df['Age'] = current_year - df['YearBuilt']
df['Land_to_distance'] = df['Landsize'] / (df['Distance'] + 1)
df['Total_rooms'] = df['Bedroom2'] + df['Bathroom']

# Cap extreme landsize
cap_landsize = df['Landsize'].quantile(0.99)
df['Landsize'] = np.where(df['Landsize'] > cap_landsize, cap_landsize, df['Landsize'])

df = df.drop(columns=['YearBuilt', 'Car', 'BuildingArea', 'HasGarage', 'Area_per_bedroom', 'Bedroom2', 'Bathroom', 'Landsize'], errors='ignore')

# Target encode Suburb
suburb_price_map = df.groupby('Suburb')['Price'].mean()
df['Suburb_encoded'] = df['Suburb'].map(suburb_price_map)
